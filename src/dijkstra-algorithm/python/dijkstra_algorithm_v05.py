...
def find_lowest_cost_node(
    ...
    for node, cost in node_to_cost.items():
        if node in nodes_processed:
            continue
        if min_node is None or cost < min_cost:
            min_cost = cost
            min_node = node
...
def dijkstra_shortest_path(
    ...
    node_to_parent: Dict[str, Optional[str]] = {start_node: None}
    node_to_cost: Dict[str, int] = {start_node: 0}
    current_node = start_node
    nodes_processed = set()

    while current_node:
        for neighor_node, cost in graph[current_node].items():
            if (neighor_node not in node_to_cost) or (
                node_to_cost[current_node] + cost < node_to_cost[neighor_node]
            ):
                node_to_cost[neighor_node] = node_to_cost[current_node] + cost
                node_to_parent[neighor_node] = current_node
