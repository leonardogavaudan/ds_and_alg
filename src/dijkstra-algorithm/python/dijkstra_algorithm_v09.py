...
def dijkstra_shortest_path(
    ...
    while queue:
        ...
        for neighor_node, neighbor_cost in graph[current_node].items():
            new_cost = node_to_cost[current_node] + neighbor_cost

            if (
                neighor_node not in node_to_cost
                or new_cost < node_to_cost[neighor_node]
            ):
                node_to_cost[neighor_node] = new_cost
                node_to_parent[neighor_node] = current_node
                heapq.heappush(queue, (new_cost, neighor_node))
